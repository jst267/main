...
1. ★ 사업관리담당자는 객실사업자 정보를 등록할 수 있다.
1. ★ 객실평가 정보가 등록되면 객실사업자 평가가 연계해서 변경된다.
비기능적 요구사항
1. 트랜잭션
...
## 추가 기능적 요구사항
- 고객은 룸서비스를 신청할 수 있다
- 룸서비스는 체크아웃하면서 결제가 진행된다
- 신청한 룸서비스가 없으면 결제 없이 체크아웃이 진행된다.
- 고객이 룸서비스를 신청시 주문정보가 전달된다.
- 룸서비스 준비가 완료되면 주문정보가 '완료' 처리된다.
- 룸서비스 준비가 완료되면 객실정보에 룸시버스 가격이 등록된다.

## 기존 비기능적 요구사항
트랜잭션

결제가 되지 않은 예약건은 아예 예약이 완료되지 않아야 한다 Sync 호출 장애격리

결제시스템이 수행되지 않더라도 객실 예약은 365일 24시간 받을 수 있어야 한다 Async (event-driven), Eventual Consistency

결제시스템이 과중되면 사용자를 잠시동안 받지 않고 결제를 잠시후에 하도록 유도한다 Circuit breaker, fallback 성능
...
## 추가 비기능적 요구사항
트랜잭션
룸서비스를 진행하였을 경우 결제가 되야만 체크아웃을 진행할 수 있다.
-> 룸서비스 결제 Sync 처리 후 체크아웃 수행

룸서비스시스템이 수행되지 않더라도 이외의 서비스는 365일 24시간 받을 수 있어야 한다 Async (event-driven), Eventual Consistency

고객이 객실관리시스템에서 신청한 룸서비스 가격을 예약시스템(프론트엔드)에서 확인할 수 있어야 한다 CQRS

마이크로 서비스를 넘나드는 시나리오에 대한 트랜잭션 처리
체크아웃 시 결제처리: 결제가 완료되지 않은 체크아웃은 절대 받지 않는다는 경영자의 원칙에 따라, ACID 트랜잭션 적용. 주문와료시 결제처리에 대해서는 Request-Response 방식 처리
